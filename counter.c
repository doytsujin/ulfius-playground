...
redisContext *redisConn() {
  ...
  const int redis_port =
      (getenv("REDIS_PORT") == NULL || atoi(getenv("REDIS_PORT")) == 0)
          ? 6379
          : atoi(getenv("REDIS_PORT"));
...
int callback_health(const struct _u_request *request,
                    struct _u_response *response, void *user_data) {
...
// to increment by one
int callback_count(const struct _u_request *request,
                   struct _u_response *response, void *user_data) {
...
int callback_metrics(const struct _u_request *request,
                     struct _u_response *response, void *user_data) {
  ...
  if (reply->type == REDIS_REPLY_ERROR) {
    y_log_message(Y_LOG_LEVEL_ERROR, "Error collecting metrics : %s",
                  reply->str);
    ulfius_set_string_body_response(response, 429, "BAD");
  } else {
    response_message =
        msprintf("global_count %s\nlocal_count %lld\nlocal_ping %lld",
...
int main(void) {
  ...
  ulfius_add_endpoint_by_val(&instance, "GET", "/count", NULL, 0,
                             &callback_count, NULL);
  ulfius_add_endpoint_by_val(&instance, "GET", "/health", NULL, 0,
                             &callback_health, NULL);
  ulfius_add_endpoint_by_val(&instance, "GET", "/metrics", NULL, 0,
                             &callback_metrics, NULL);
  ...
  if (ulfius_start_framework(&instance) == U_OK) {
    y_log_message(Y_LOG_LEVEL_INFO, "Start framework on port %d",
                  instance.port);
